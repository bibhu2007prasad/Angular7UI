...
import { ProcessMandateInWard } from '../../Models/ProcessMandateInWard/processMandateInWard';
import { CorporateProcInWard } from '../../Models/ProcessMandateInWard/corporateProcInWard';
import { SponserBankProcInWard } from '../../Models/ProcessMandateInWard/sponserBankProcInWard';
import { DownloadOldMandateProcInWardService } from '../../Services/download-old-mandate-proc-in-ward.service';
...
@Component({
  selector: 'app-processed-mandates-onus',
  templateUrl: './processed-mandates-onus.component.html',
  styleUrls: ['./processed-mandates-onus.component.css']
})
export class ProcessedMandatesOnusComponent implements OnInit {

  ProcessMandateFormInWord: FormGroup; HeaderArray;corporate:CorporateProcInWard; sponserBank:SponserBankProcInWard;
  BindAllData: ProcessMandateInWard; TotalCount = 0; dataArray: Array<ProcessMandateInWard> = [];
  ...
  SponserBankCode: any;
  length: any;
  ZipDownloadArray: Array<ProcessMandateInWard> = [];
  constructor(public myservice: DownloadOldMandateProcInWardService, private formBuilder: FormBuilder) { 
    this.corporate = new CorporateProcInWard();
      this.corporate.dataList = [];

      this.sponserBank = new SponserBankProcInWard();
      this.sponserBank.dataList = [];

      this.BindAllData = new ProcessMandateInWard();
  ...
  ngOnInit() {
    this.ProcessMandateFormInWord = this.formBuilder.group({
      FromDate: [''],
      ToDate: [''],
      EntityId: [0],
      sponsorbankcode: [0],
...
SearchFunction(FromDate, ToDate) {
  ...
  const datat = this.ProcessMandateFormInWord.value;
    this.Preloader = true;
